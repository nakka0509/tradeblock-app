...
// --- High Conversion Double Tap Prompt ---
const DoubleTapPrompt: React.FC<{ lang: string }> = ({ lang }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // Spring animation for punchy entrance
  const scale = spring({
    fps,
    frame,
    config: {
  ...
  // Continuous pulsing after entrance to maintain attention
  const pulse = interpolate(Math.sin((frame - 15) * 0.2), [-1, 1], [0.97, 1.03]);
  const finalScale = frame > 15 ? pulse : scale;

  const text = lang === 'en' ? 'Double Tap if you relate' :
    lang === 'zh-CN' ? '有同感的双击屏幕' :
      'あるあると思ったら2回タップ';
  ...
  return (
    <div style={{
      ...
      {/* Sound effect synced with the pop-up */}
      <Audio src={staticFile('assets/part3/決定ボタンを押す40.mp3')} volume={0.8} />

      <div style={{
        transform: `scale(${finalScale})`,
        backgroundColor: '#FF004F', // TikTok/Instagram vibrant red
        padding: '24px 50px',
        borderRadius: '60px',
        border: '6px solid white',
        boxShadow: '0 20px 50px rgba(255, 0, 79, 0.6), 0 0 0 10px rgba(255,255,255,0.1)',
        display: 'flex',
        alignItems: 'center',
        gap: '16px'
      }}>
        <div style={{
          fontSize: lang === 'en' ? 42 : 52,
          fontWeight: '900',
          color: 'white',
...
export const TradeBlockAruaruV2Carousel: React.FC<{
  hookText: string;
  empathyText: string;
  ...
  return (
    <AbsoluteFill style={{
      ...
      {/* Global Interactive Elements that span parts */}
      <Sequence from={45} durationInFrames={Math.max(10, slide1Duration + slide2Duration - 45)}>
